...
@RestController
@RequestMapping("/api/v1/landlord_login")
public class AuthController {
    ...
    @Operation(
    		summary = "Login logic in landlord login authcontroller",
    		description = "Login logic in landlord login"
    		)
    @ApiResponse(
    		responseCode = "201",
    ...
    public ResponseEntity<?> login(@RequestBody Map<String,String> body, HttpServletRequest req) {
        ...
        if (user == null) {
        	...
        	LOGGER.error("Invalid Credentials in landlord login controller");
            return ResponseEntity.status(401).body(Map.of("error","invalid credentials"));
        }

        if (userService.isLocked(user)) {
        	LOGGER.error("Account is locked due to excessive incorrect tries in landlord login controller");
        ...
        LOGGER.info("Landlord Login component is being successfuly executed");
        return ResponseEntity.ok(Map.of("message","otp_sent"));
    }
    @Operation(
    		summary = "Verify otp logic in landlord login authcontroller",
    		description = "Verify otp logic in landlord login"
    		)
    @ApiResponse(
    		responseCode = "201",
    		description = "HTTP STATUS 201 created"
    		)
 // http:localhost:8087/api/v1/landlord_login/verify-otp
    ...
    public ResponseEntity<?> verifyOtp(@RequestBody Map<String,String> body, HttpServletRequest req) {
        ...
        LOGGER.info("Landlord Login Otp verification is being successfully executed");
        return ResponseEntity.ok(Map.of("message","authenticated"));
    }
    @Operation(
    		summary = "Resend Otp logic in landlord login authcontroller",
    		description = "Resend Otp logic in landlord login"
    		)
    @ApiResponse(
    		responseCode = "201",
    		description = "HTTP STATUS 201 created"
    		)
 // http:localhost:8083/api/v1/landlord_login/resend-otp
    ...
    @Operation(
    		summary = "Logout logic in landlord login authcontroller",
    		description = "Logout logic in landlord login"
    		)
    @ApiResponse(
    		responseCode = "201",
    		description = "HTTP STATUS 201 created"
    		)
 // http:localhost:8087/api/v1/landlord_login/logout
    @PostMapping("/logout")
    ...
    @Operation(
    		summary = "Sucessful login logic in landlord login authcontroller",
    		description = "successful login logic in landlord login"
    		)
    @ApiResponse(
    		responseCode = "201",
    		description = "HTTP STATUS 201 created"
    		)
 // http:localhost:8087/api/v1/landlord_login/dashboard
